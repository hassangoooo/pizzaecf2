...
function filterTaches(data) {
  ...
  nomPizza.innerHTML = "";
  data.forEach(pizza => {
    if (filtreActif === 0 || (filtreActif === 1 && pizza.nom_base === "tomate") || (filtreActif === 2 && pizza.nom_base === "creme")) {
...
function afficherLesdonnees(data) {
  data.forEach(pizza => {
    ...
    h3.textContent = pizza.nom_pizza;
    h2.textContent = pizza.nom_base;
    ...
    pizza.ingredients.split(',').forEach(ingredient => {
    ...
    tarifDev.textContent = `${pizza.prix_pizza}€`;
...
function ajouterAuPanier(pizza) {
  panier.push(pizza);
  montantTotal += parseFloat(pizza.prix_pizza);
...
function afficherPanier() {
  ...
  if (panier.length === 0) {
    ...
    panier.forEach((pizza, index) => {
      ...
      prix.textContent = `${pizza.prix_pizza}€`;
...
function supprimerDuPanier(index) {
  montantTotal -= parseFloat(panier[index].prix_pizza);
